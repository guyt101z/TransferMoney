                                ...
                                <table class="table table-striped table-bordered table-hover sortable" id="dataTables-example">
                                 ...
                                 <?php
									...
									$select4=$db->query("SELECT tblproductsbranch.ProductID,
											...
											WHERE 
											(tblproducts.ProductName LIKE '%".$sarchprd."%' OR tblproducts.ProductCode LIKE '%".$sarchprd."%' ) AND 
											tblproducts_buy.BuyDate BETWEEN '".$txtFrom."' AND '".$txtTo."'
									...
									$select3=$db->query("SELECT tblproductsbranch.ProductID, 
													...
													ON tblproducts.ProductID = tblproductsbranch.ProductID
													WHERE 
													(tblproducts.ProductName LIKE '%".$sarchprd."%' 
													OR tblproducts.ProductCode LIKE '%".$sarchprd."%' ) AND
													tblproducts_buy.BuyDate BETWEEN '".$txtFrom."' AND '".$txtTo."'
													AND tblproductsbranch.Qty = 1 ");
									// Show Product  sell					
									$select2=$db->query("SELECT tblproductsbranch.ProductID, 
													tblproductsbranch.BuyPrice, 
													tblproductsbranch.OtherCost AS TCost, 
													tbl_customerorderdetail.SalePrice,
													tblproductsbranch.Qty,
													tblproductsbranch.Decription
												 FROM tblproductsbranch
												INNER JOIN tbl_customerorderdetail
...
COALESCE((tbl_customerorderdetail.SalePrice),0)-(COALESCE((tblproducts_buydetail.BuyPrice),0)+COALESCE((tblproductsbranch.OtherCost),0)) AS totalIncome, COALESCE((tblproductsbranch.OtherCost),0) AS OtherCost	
												FROM tbl_customerorder
												INNER JOIN tbl_customerorderdetail
													ON tbl_customerorder.CustomerOrderID = tbl_customerorderdetail.CustomerOrderID
												INNER JOIN tblproducts
...
COALESCE((tbl_customerorderdetail.SalePrice),0)-(COALESCE((tblproducts_buydetail.BuyPrice),0)+COALESCE((tblproductsbranch.OtherCost),0)) AS totalIncome		
											...
											echo '<tr class="odd gradeX">';
															/*$rowselect3=$db->dbCountRows($select2);
															if($rowselect3>0){
																$i = 1;
																$TotalBuyprice = 0;
																while($row0=$db->fetch($select2)){
																	
																	$TotalBuyprice += $row0->BuyPrice;
																	
																}
																echo '<td colspan="2" class="text-left">Instock = '.$select2.'
																($'.$TotalBuyprice.')</td>';
															}
															else
															{
																echo '<td colspan="2" class="text-left">Products Not Sole = 0($0)</td>';
															}*/
															...
															if($rowselect2>0){
																...
																while($row0=$db->fetch($select2)){
																	...
																	$TotalSaling += $row0->SalePrice;
																	$TotalBuying += $row0->BuyPrice;
																	$TotalOtherCost += $row0->TCost;
																}
																echo '<td colspan="2" class="text-left">Buying = '.$rowselect2.'
																($'.$TotalBuying.')</td>';
																echo '<td colspan="2" class="text-left">Salling = '.$rowselect2.'
																($' .$TotalSaling.')</td>';
																echo '<td colspan="2" class="text-left">OtherCost = '.$rowselect2.'
																($' .$TotalOtherCost.')</td>';
															}
															else
															{
											...
											if($rowselect4>0){
												...
												while($row4=$db->fetch($select4)){
													...
													echo '<td class="col-md-1 text-left">'.$row4->ProductCategoryName.'</td>
														...
														<td class="col-md-1 text-right"> - </td>';
														echo '</tr>';
														$TotalBuying += (float)$row4->BuyPrice + (float)$row4->BuyPrice;		
												}
											
												echo '<td colspan="8" class="text-right"> Total</td>
													  <td class="text-right"> $ '.round($TotalBuying,2).'
													  </td>
												</tr>';
...
<!-- New User -->
               <div class="modal fade" id="NewUser" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                  ...
                  <div class="modal-content">
                      ...
                      <form enctype="multipart/form-data" action="rptSaleAdvance.php">
